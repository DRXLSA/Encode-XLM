...
def get_parser():
    ...
    parser.add_argument("--use_encode_model", type=bool_flag, default=False,
                        help="use a pretrained encode model")
    
    parser.add_argument("--encode_model", type=str, default="",
                        help="Reload a encode model")
    # model parameters
...
def main(params):
    ...
    if params.use_encode_model and os.path.isfile(params.encode_model):
        encoder_only = params.encoder_only
        reload_model = params.reload_model
        params.encoder_only = True
        params.reload_model = params.encode_model

        encode_model = build_model(params, data['dico'])
        encode_model.eval()

        params.encoder_only = encoder_only
        params.reload_model = reload_model
    else:
        encode_model = None

    # build trainer, reload potential checkpoints / build evaluator
    if params.encoder_only:
        trainer = SingleTrainer(model, data, params, encode_model)
        evaluator = SingleEvaluator(trainer, data, params)
    else:
        trainer = EncDecTrainer(encoder, decoder, data, params,encode_model)
