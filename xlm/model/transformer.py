...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output,with_embeddings):
        """
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None,encode_tensor=None):
        """
        Inputs:
        ...
        """
        cts_loss_mean = 0
        if self.is_encoder and encode_tensor is not None:#如果是encoder，才改写tensor
            #print("use encode")
            tensor = encode_tensor.transpose(0,1)#encode_model('fwd', x=x, lengths=lengths, langs=langs, causal=False).detach().transpose(0,1)
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        if cur_len == max_len:
            generated[-1].masked_fill_(unfinished_sents.byte().bool(), self.eos_index)
