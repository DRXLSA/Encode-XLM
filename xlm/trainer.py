...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('MLM-cts-%s' % l,[]) for l in params.langs ] +
            [('MLM-cts-%s-%s' % (l1, l2), []) for l1, l2 in data['para'].keys()] +
            [('AE-cts-%s' % l,[]) for l in params.langs ] +
            [('AE-cts-%s-%s' % (l1, l2), []) for l1, l2 in data['para'].keys()] +
    ...
    def set_parameters(self):
        ...
        named_params = []
        for name in self.MODEL_NAMES:
            print([k for k, p in getattr(self, name).named_parameters() if p.requires_grad])
    ...
    def word_shuffle_with_tensor(self, x, l, tensor):
        """
        Randomly shuffle input words.
        """
        if self.params.word_shuffle == 0:
        ...
        for i in range(l.size(0)):
            ...
            #deal tensor
            tensor2[:l[i] - 1, i].copy_(tensor[:l[i] - 1,i][torch.from_numpy(permutation)])

        return x2, l, tensor2


    def word_dropout_with_tensor(self, x, l, tensor):
        """
        Randomly drop input words.
        """
        if self.params.word_dropout == 0:
        ...
        #deal tensor
        tensor2 = torch.zeros(x2.shape[0],x2.shape[1],tensor.shape[2])
        for i in range(l.size(0)):

            tensor2[:keep[:l[i] - 1,i].sum(),i] = tensor[np.nonzero(keep[:l[i] - 1,i])[0],i]
            tensor2[keep[:l[i] - 1,i].sum(),i] = tensor[l[i]-1,i] #eos



        return x2, l2, tensor2



    def add_noise_and_generate_tensor(self, words, lengths,langs,encode_model):

        tensor = encode_model('fwd', x=words.cuda(), lengths=lengths.cuda(), langs=langs.cuda(), causal=False).detach()

        #add_noise
        words, lengths, tensor = self.word_shuffle_with_tensor(words.cpu(), lengths.cpu(), tensor.cpu())
        words, lengths, tensor = self.word_dropout_with_tensor(words.cpu(), lengths.cpu(), tensor.cpu())

        return words,lengths,tensor
    ...
    def generate_batch(self, lang1, lang2, name):
        ...
        if lang2 is None:
            ...
            x, lengths = self.word_shuffle(x, lengths) #特别处理，希望得到适应denoise的encode model
    ...
    def mlm_step(self, lang1, lang2, lambda_coeff):
        ...
        tensor_tr = tensor.transpose(0,1)
        tokens_num = 0
        for i in range(tensor.shape[0]):
            tensor_step = tensor[i].repeat([1,tensor.shape[0]]).reshape(-1,tensor.shape[0],1024)
            

            step_loss = criterion(tensor_step, tensor_tr)
        ...
        cts_loss_mean = cts_loss / tokens_num
        self.stats[('MLM-cts-%s' % lang1) if lang2 is None else ('MLM-cts-%s-%s' % (lang1, lang2))].append(cts_loss_mean.item())
        


        _, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=False)
        self.stats[('MLM-%s' % lang1) if lang2 is None else ('MLM-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss + cts_loss_mean
...
class SingleTrainer(Trainer):
    ...
    def __init__(self, model, data, params,encode_model):
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params,encode_model):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        x1_tensor = None
        if params.use_encode_model :
            #x1,len1,x1_tensor = self.add_noise_and_generate_tensor(x1,len1,langs1,self.encode_model)
            x1_tensor = self.encode_model('fwd', x=x1, lengths=len1, langs=langs1, causal=False).detach()                                                                                   
            x1_tensor.cuda()
        x1, len1, langs1, x2, len2, langs2, y = to_cuda(x1, len1, langs1, x2, len2, langs2, y)
        # encode source sentence
        enc1 = self.encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False,encode_tensor=x1_tensor)
        enc1 = enc1.transpose(0, 1)

        # decode target sentence
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
        ''' 
        tensor = self.encode_model('fwd', x=x1, lengths=len1, langs=langs1, causal=False).transpose(0,1)
        cts_loss = 0
        criterion = nn.CosineSimilarity(dim=2)
        tensor_tr = tensor.transpose(0,1)
        tokens_num = 0
        ...
        cts_loss_mean = cts_loss / tokens_num
        '''
        #self.stats[('AE-cts-%s' % lang1) if lang1==lang2 else ('AE-cts-%s-%s' % (lang1, lang2))].append(cts_loss_mean.item())
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        with torch.no_grad():
            ...
            enc1 = _encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False,encode_tensor=x1_tensor)
            enc1 = enc1.transpose(0, 1)
        ...
        x2_tensor = None
        if params.use_encode_model :
            x2_tensor = self.encode_model('fwd', x=x2, lengths=len2, langs=langs2, causal=False).detach()
       
        enc2 = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, causal=False,encode_tensor=x2_tensor)
