...
CODES=80000     # number of BPE codes
...
TOKENIZE_ZH=$TOOLS_PATH/tokenize_zh.py
# fastBPE
...
if [ "$SRC" == "en" -a "$TGT" == "zh" ]; then
  PARA_SRC_VALID=$PARA_PATH/dev-enzh/newsdev2017-enzh-src.en
  PARA_TGT_VALID=$PARA_PATH/dev-enzh/newsdev2017-enzh-ref.zh
  PARA_SRC_TEST=$PARA_PATH/dev-enzh/newstest2019-zhen-ref.en
  PARA_TGT_TEST=$PARA_PATH/dev-enzh/newstest2019-zhen-src.zh
fi
...
SRC_NORM=$MONO_PATH/$SRC/all.$SRC.nor
# tokenize data
if ! [[ -f "$SRC_TOK" ]]; then
  echo "Tokenize $SRC monolingual data..."
  if [ "$SRC" == "zh" ]; then
    cat $SRC_RAW | $REPLACE_UNICODE_PUNCT |  $NORM_PUNC -l $SRC | $REM_NON_PRINT_CHAR > $SRC_NORM
    $TOKENIZE_ZH $SRC_NORM $SRC_TOK
  else
...
TGT_NORM=$MONO_PATH/$TGT/all.$TGT.nor
if ! [[ -f "$TGT_TOK" ]]; then
  echo "Tokenize $TGT monolingual data..."
  if [ "$TGT" == "zh" ]; then
    cat $TGT_RAW | $REPLACE_UNICODE_PUNCT |  $NORM_PUNC -l $TGT | $REM_NON_PRINT_CHAR > $TGT_NORM
    $TOKENIZE_ZH $TGT_NORM $TGT_TOK
  else
...
wget -c http://data.statmt.org/wmt20/translation-task/dev.tgz -O dev-$SRC$TGT.tgz

echo "Extracting parallel data..."
if [ ! -d "dev-$SRC$TGT" ]; then
    mkdir dev-$SRC$TGT
    tar -xzf dev-$SRC$TGT.tgz -C ./dev-$SRC$TGT --strip-components 1
fi
...
if [ "$TGT" == "zh" ]; then
  $INPUT_FROM_SGM < $PARA_TGT_VALID.sgm | $REPLACE_UNICODE_PUNCT |  $NORM_PUNC -l $TGT | $REM_NON_PRINT_CHAR > $PARA_TGT_VALID.NORM
  $TOKENIZE_ZH $PARA_TGT_VALID.NORM $PARA_TGT_VALID
else
...
if [ "$TGT" == "zh" ]; then
  $INPUT_FROM_SGM < $PARA_TGT_TEST.sgm | $REPLACE_UNICODE_PUNCT |  $NORM_PUNC -l $TGT | $REM_NON_PRINT_CHAR > $PARA_TGT_TEST.NORM
  $TOKENIZE_ZH $PARA_TGT_TEST.NORM $PARA_TGT_TEST
else
